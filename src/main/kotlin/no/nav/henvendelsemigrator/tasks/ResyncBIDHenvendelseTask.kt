...
class ResyncBIDHenvendelseTask(
    val henvendelseDb: HealthcheckedDataSource,
    val producer: KafkaProducer<String, String>
) : SimpleTask() {
    override val name: String = "BUGFIX - Resynk henvendelser knyttet til BID"
    override val description: String = """
        Resynkroniserer alle henvendelser knyttet til BID
    """.trimIndent()
    ...
    override suspend fun runTask() {
        ...
        val query = "SELECT henvendelse_id FROM HENVENDELSE WHERE type in ($henvendelsetyper) AND STATUS = 'FERDIG' AND journalforttema = 'BID'"
    ...
    override fun toHealtchCheck() = Healthcheck {
        ...
        try {
            executeQuery(
                henvendelseDb,
                "SELECT henvendelse_id FROM HENVENDELSE WHERE type in ($henvendelsetyper) AND (behandlingsid = ? OR behandlingsid = ?)",
                {
                ...
                { rs ->
                    ...
                    if (henvendelseId == null) {
                        ...
                        HealthcheckResult.Ok(
                            name,
                            System.currentTimeMillis() - start,
                            "Fikk hentet ut henvendelse_id fra henvendelse"
                        )
                    }
                }
            )
